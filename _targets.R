...
list(
  ...
  tar_target(
    multivar_allDLI_nointer_penal, # name is non case sensitive!
    jm(
      Surv_object = cox_all_dli,
      Mixed_objects = long_submodels,
      time_var = "intSCT2_5",
      ...
      priors = list(
        # Local ridge priors for each alpha ~ N(0, 1/2) - change 3
        Tau_alphas = lapply(seq_len(6), function(alpha) matrix(data = 0.5))
      )
    )
  ),
  tar_target(
    multivar_allDLI_inter_penal, # name is non case sensitive!
    jm(
      Surv_object = cox_all_dli,
      Mixed_objects = long_submodels,
      time_var = "intSCT2_5",
      ...
      priors = list(
        # Local ridge priors for each alpha ~ N(0, 1/2) - change 3
        Tau_alphas = lapply(seq_len(12), function(alpha) matrix(data = 0.5))
      )
    )
  ),
  ...
  # Run reduced models
  tar_target(
    multivar_DLIgvhd_inter, # name is non case sensitive!
    jm(
      Surv_object = cox_dli_gvhd,
      Mixed_objects = long_submodels,
      time_var = "intSCT2_5",
      functional_forms = list(
        "CD4_abs_log" = ~ value(CD4_abs_log):(
